...
class DependenciasRepository {
  ...
  static async CreateDependencia(
    nombre,
    custom_id,
    delegacion_id,
    funcionario_id,
    ...
    try {
      ...
      const existingDependencia = await Dependencias.findOne({
        where: { nombre: nombre },
      });
  ...
  static async updateDependencia(
    id,
    nombre,
    custom_id,
    delegacion_id,
    ...
    try {
      ...
      dependencia.tipodependenciaId =
        tipodependenciaId || dependencia.tipodependenciaId;
  ...
  static async getUtilsDep() {
    ...
    try {
      const dependencias = await Dependencias.findAll({
        attributes: ["id", "nombre", "custom_id", "delegacion_id", "funcionario_id", "tipodependenciaId"],
      });
      ...
      const delegaciones = await Delegacion.findAll({
        attributes: ["id", "nombre", "custom_id", "fun_delegacionId"],
      });
      const tiposDependencia = await TipoDependencia.findAll({
        attributes: ["id", "nombre"],
      });
      ...
      response.message = "Datos de utilidades obtenidos correctamente";
      response.type_of_response = TypeOfResponse.SUCCESS;
      return response;
    } catch (error) {
